...
vi.mock('../../src/components/ScheduleView', () => ({
    ScheduleView: ({ currentDate, setCurrentDate }: { 
        currentDate: Dayjs; 
        setCurrentDate: (date: Dayjs) => void;
    }) => (
        <div data-testid="schedule-view">
            <span data-testid="current-date">{currentDate.format('YYYY-MM-DD')}</span>
            <button 
...
vi.mock('../../src/components/TransferView', () => ({
    TransferView: () => <div data-testid="transfer-view">Transfer View Content</div>,
}));
...
describe('MainTabs Component', () => {
    ...
    describe('Rendering', () => {
        ...
        it('should render the correct tab content based on active tab', () => {
            const { rerender } = render(<MainTabs {...defaultProps} activeTab="today" />);
            expect(screen.getByTestId('today-view')).toBeInTheDocument();
    ...
    describe('Tab Navigation', () => {
        it('should call onTabChange when tab is clicked', async () => {
            ...
            render(<MainTabs {...defaultProps} onTabChange={mockOnTabChange} />);
    ...
    describe('Props Synchronization', () => {
        ...
        it('should pass props correctly to child components', () => {
            ...
            expect(screen.getByTestId('current-date')).toHaveTextContent('2024-01-10');
        });

        it('should handle currentDate changes from ScheduleView', () => {
            const mockSetCurrentDate = vi.fn();
            render(<MainTabs {...defaultProps} setCurrentDate={mockSetCurrentDate} activeTab="schedule" />);
            
            const setDateButton = screen.getByTestId('set-date-button');
            fireEvent.click(setDateButton);
            
            expect(mockSetCurrentDate).toHaveBeenCalledWith(dayjs('2024-01-15'));
        });
    });
    ...
    describe('Today Click Functionality', () => {
        it('should handle today click from TodayView', () => {
            const mockSetCurrentDate = vi.fn();
            render(<MainTabs {...defaultProps} setCurrentDate={mockSetCurrentDate} activeTab="today" />);
            ...
            expect(calledWith.format('YYYY-MM-DD')).toBe(dayjs().format('YYYY-MM-DD'));
        });
    ...
    describe('Component Structure', () => {
        it('should have correct CSS classes and structure', () => {
            ...
            const tabsContainer = screen.getByRole('tablist').closest('.col-12.mb-4');
    ...
    describe('Integration with dayjs', () => {
        it('should handle dayjs date objects correctly', () => {
            ...
            render(<MainTabs {...defaultProps} currentDate={specificDate} activeTab="schedule" />);
            
            expect(screen.getByTestId('current-date')).toHaveTextContent('2024-06-15');
        });
    ...
    describe('Error Handling', () => {
        ...
        it('should handle setCurrentDate callback errors gracefully', () => {
            ...
            render(<MainTabs {...defaultProps} setCurrentDate={errorCallback} activeTab="today" />);
